 ...
 * This source code is licensed under the Apache-2.0 license found in the
 * LICENSE file in the root directory of this source tree.
 */!function(e){e.SMALL="sm",e.MEDIUM="md",e.LARGE="lg"}(v||(v={}));
/**
 * @license
 *
 ...
 * LICENSE file in the root directory of this source tree.
 */
let u=class extends(a(l(n))){constructor(){super(...arguments),this.backwardText="Previous page",this.page=1,this.formatLabelText=({count:e})=>`Page number, of ${e} page${e<=1?"":"s"}`,this.formatStatusWithDeterminateTotal=({start:e,end:t,count:o})=>`${e}–${t} of ${o} item${o<=1?"":"s"}`,this.formatStatusWithIndeterminateTotal=({start:e,end:t,count:o})=>null==t?`Item ${e}–`:`${e}–${t} item${o<=1?"":"s"}`,this.formatSupplementalText=({count:e})=>this.pagesUnknown||!this.totalItems?"page":`of ${e} page${e<=1?"":"s"}`,this.itemsPerPageText="Items per page:",this.disabled=!1,this.forwardText="Next page",this.pageInputDisabled=!1,this.pageSize=10,this.pagesUnknown=!1,this.size=v.MEDIUM,this.start=0,this.totalPages=1}_handleSlotChange({target:e}){e.assignedNodes().filter((e=>e.nodeType!==Node.TEXT_NODE||e.textContent.trim())).forEach((e=>{this._pageSizeSelect.appendChild(e)}))}_renderStatusText(){const{start:e,pageSize:t,totalItems:o,pagesUnknown:s,formatStatusWithDeterminateTotal:n,formatStatusWithIndeterminateTotal:i}=this;return(null==o||s?i:n)({start:e+1,end:Math.min(e+t,null==o?1/0:o),count:o})}_handleUserInitiatedChangeStart(e){this.start=e,this.dispatchEvent(new CustomEvent(this.constructor.eventChangeCurrent,{bubbles:!0,composed:!0,detail:{page:this.page,pageSize:this.pageSize}}))}_handleUserInitiatedPageSizeChange(){this.dispatchEvent(new CustomEvent(this.constructor.eventPageSizeChanged,{bubbles:!0,composed:!0,detail:{page:this.page,pageSize:this.pageSize}}))}_handleClickPrevButton(){var e;const{start:t,pageSize:o}=this;this.page--;const s=Math.max(t-o,0);if(s!==t&&this._handleUserInitiatedChangeStart(s),1===this.page){const{selectorForwardButton:t}=this.constructor;(null===(e=this.shadowRoot)||void 0===e?void 0:e.querySelector(`[button-class-name*=${t}]`)).focus()}}_handleClickNextButton(){var e;const{start:t,pageSize:o,totalItems:s}=this;this.page++;const n=t+o;if(n<(null==s?1/0:s)&&this._handleUserInitiatedChangeStart(n),this.page===this.totalPages){const{selectorPreviousButton:t}=this.constructor;(null===(e=this.shadowRoot)||void 0===e?void 0:e.querySelector(`[button-class-name*=${t}]`)).focus()}}_handleChangeSelector(e){const{value:t}=e.detail,{totalItems:o,pageSize:s}=this;e.composedPath()[0]===this._pageSizeSelect?(this.pageSize=parseInt(t),this.totalPages=s>0?Math.ceil(o/s):o,this.page=1,this.start=0,this._handleUserInitiatedPageSizeChange()):(this.page=t,this._handleUserInitiatedChangeStart((t-1)*s))}updated(e){const{page:t,pageSize:o,totalItems:s}=this,{selectorPageSizesSelect:n,selectorPagesSelect:i}=this.constructor;e.has("pageSize")&&(this.shadowRoot.querySelector(n).value=o),(e.has("pageSize")||e.has("start"))&&(this.totalPages=o>0?Math.ceil(s/o):s,this.shadowRoot.querySelector(i).value=this.page.toString()),e.has("page")&&this._handleUserInitiatedChangeStart((t-1)*o)}render(){const{page:t,disabled:n,forwardText:i,backwardText:c,itemsPerPageText:r,pageInputDisabled:a,pageSize:l,pageSizeInputDisabled:p,pagesUnknown:g,size:v,start:b,totalItems:u,totalPages:h,_handleClickPrevButton:f,_handleClickNextButton:m,_handleSlotChange:y,formatLabelText:_,formatSupplementalText:x}=this,{isLastPage:z=b+l>=u}=this,k=n||0===b,w=n||z,S={[`${s}--btn`]:!0,[`${s}--btn--icon-only`]:!0,[`${s}--pagination__button`]:!0,[`${s}--pagination__button--backward`]:!0,[`${s}--pagination__button--no-index`]:k,[`${s}--btn--${v}`]:!0,[`${s}--btn--ghost`]:!0},$={[`${s}--btn`]:!0,[`${s}--btn--icon-only`]:!0,[`${s}--pagination__button`]:!0,[`${s}--pagination__button--forward`]:!0,[`${s}--pagination__button--no-index`]:w,[`${s}--btn--${v}`]:!0,[`${s}--btn--ghost`]:!0},q=Object.entries(S).filter((([,e])=>!0===e)).map((([e])=>e)).join(" "),T=Object.entries($).filter((([,e])=>!0===e)).map((([e])=>e)).join(" ");return o`
      <div class="${s}--pagination__left">
        <label for="select" class="${s}--pagination__text"
          ><slot name="label-text">${r}</slot></label
        >
        <cds-select
          ?disabled=${n||p}
          id="page-size-select"
        ...
        <span
          class="${s}--pagination__text ${s}--pagination__items-count"
          >${this._renderStatusText()}</span
        >
      </div>
      <div class="${s}--pagination__right">
        <label for="select" class="${s}--label ${s}--visually-hidden">
          ${_({count:h})}
        </label>
        ${g||!u?o`
              <span
                class="${s}--pagination__text ${s}--pagination__page-text"
                >${x({count:h})}</span
              >
              ...
              <cds-select
                ...
                inline
                value="${t}">
                ${Array.from(new Array(h)).map(((e,t)=>o`
                    <cds-select-item value="${t+1}">
                      ${t+1}
                    </cds-select-item>
                  `))}
              </cds-select>
            `:o`
              ...
              <span class="${s}--pagination__text"
                >${x({count:h})}</span
              >
            `}

        <div class="${s}--pagination__control-buttons">
          <cds-button
            pagination
            size="${v}"
            ?disabled="${k}"
            ...
            tooltip-text="${c}"
            @click="${f}">
            ${(({children:t,...o}={})=>e`<svg focusable="false" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" fill="currentColor" ...="${d(o)}" aria-hidden="true" width="16" height="16" viewBox="0 0 32 32">${t}<path d="M20 24L10 16 20 8z"></path></svg>`)({slot:"icon"})}
          </cds-button>
          <cds-button
            ...
            tooltip-text="${i}"
            @click="${m}">
            ${(({children:t,...o}={})=>e`<svg focusable="false" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" fill="currentColor" ...="${d(o)}" aria-hidden="true" width="16" height="16" viewBox="0 0 32 32">${t}<path d="M12 8L22 16 12 24z"></path></svg>`)({slot:"icon"})}
          </cds-button>
        </div>
      </div>
    `}static get selectorPagesSelect(){return`${s}-select#pages-select`}static get selectorPageSizesSelect(){return`${s}-select`}static get selectorPreviousButton(){return`${s}--pagination__button--backward`}static get selectorForwardButton(){return`${s}--pagination__button--forward`}static get eventChangeCurrent(){return`${s}-pagination-changed-current`}static get eventPageSizeChanged(){return`${s}-page-sizes-select-changed`}};u.shadowRootOptions=Object.assign(Object.assign({},n.shadowRootOptions),{delegatesFocus:!0}),u.styles=b,i([r(`${s}-select`)],u.prototype,"_pageSizeSelect",void 0),i([p(`${s}-select-selected`)],u.prototype,"_handleChangeSelector",null),i([c({attribute:"backward-text"})],u.prototype,"backwardText",void 0),i([c({type:Number,reflect:!0})],u.prototype,"page",void 0),i([c({attribute:!1})],u.prototype,"formatLabelText",void 0),i([c({attribute:!1})],u.prototype,"formatStatusWithDeterminateTotal",void 0),i([c({attribute:!1})],u.prototype,"formatStatusWithIndeterminateTotal",void 0),i([c({attribute:!1})],u.prototype,"formatSupplementalText",void 0),i([c({type:Boolean,attribute:"is-last-page"})],u.prototype,"isLastPage",void 0),i([c({attribute:"items-per-page-text"})],u.prototype,"itemsPerPageText",void 0),i([c({type:Boolean,reflect:!0})],u.prototype,"disabled",void 0),i([c({attribute:"forward-text"})],u.prototype,"forwardText",void 0),i([c({type:Boolean,attribute:"page-input-disabled"})],u.prototype,"pageInputDisabled",void 0),i([c({type:Number,attribute:"page-size",reflect:!0})],u.prototype,"pageSize",void 0),i([c({type:Boolean,attribute:"page-size-input-disabled"})],u.prototype,"pageSizeInputDisabled",void 0),i([c({attribute:"page-size-label-text"})],u.prototype,"pageSizeLabelText",void 0),i([c({type:Boolean,reflect:!0,attribute:"pages-unknown"})],u.prototype,"pagesUnknown",void 0),i([c({reflect:!0})],u.prototype,"size",void 0),i([c({type:Number})],u.prototype,"start",void 0),i([c({type:Number,attribute:"total-items"})],u.prototype,"totalItems",void 0),i([c({type:Number})],u.prototype,"totalPages",void 0),u=i([g(`${s}-pagination`)],u);
